...
## 9. `/monitor` 지도를 서버 그래프로 재구성 (완료)

두 그래프를 나란히 놓고 보니 **눈에 띄게 달랐다.** map-tool 이 만든 것과 관리자웹이
만든 것이 노드 위치·개수부터 다르다. 서버는 이제 관리자웹 쪽을 정확히 재현하므로
(§7-2, 좌표 오차 0) `/monitor` 가 map-tool 그림을 보여줄 이유가 없어졌다.

### 끊겨 있던 곳을 이었다

    STT 텍스트 → LLM 매칭 → 목적지 확정 → [끊김] → 경로 생성 → 비콘 순서 → 추적 판정

`route_engine` 은 만들어져 있었는데 **`handler.py` 가 import 조차 하지 않았다.**
목적지를 알아들어도 경로를 만들지 않았고, 경로는 `/monitor` 에서 사람이 비콘 순서를
손으로 등록해야 돌았다. `app/ws/navigation.py` 가 그 사이를 잇는다.

```
"사백칠호" → 407 → 48m / 69초 / 비콘 8개 → 추적기에 8개 등록
```

**출발점은 지금 가장 세게 잡히는 비콘**이다. 알 수 있는 단서가 그것뿐이다.
신호가 제일 센 비콘이 늘 제일 가까운 비콘은 아니지만(사람이 가리거나 반사되면
뒤바뀐다), 한 칸 어긋나도 경로는 같은 복도를 타므로 실용상 문제가 없다.
정확한 위치는 그 뒤 `PathTracker` 가 계속 판정한다.

**층은 목적지가 알려준다.** 비콘의 major 로도 알 수 있지만 랜드마크가 이미 자기
층을 들고 있어서 그쪽이 확실하다. (층간 이동 안내는 아직 다루지 않는다)

### 경로를 못 만들어도 목적지 응답은 살린다

경로 실패 이유는 여러 가지다(축척 미설정, 비콘 미등록, 길 없음, 안 잡히는 비콘).
이때 "목적지를 못 알아들었다"로 보이면 엉뚱한 곳을 고치게 되므로 `routeError` 로
따로 실어 보낸다.

### 목적지 목록을 DB 로 바꿨다

`_load_landmark_list()` 가 지도 프로젝트 **파일**을 읽고 있었다. 경로 생성은 DB 의
랜드마크 id 로 하므로 목록이 다르면 id 가 안 맞는다. DB 를 먼저 보고, 비어 있거나
붙지 않으면 예전처럼 파일로 떨어진다(서버만 띄우고 실측하던 흐름을 깨지 않으려고).

### 지도는 읽기 전용 캔버스 한 장이 됐다

| | 전 | 후 |
|---|---|---|
| `/monitor` 지도 | map-tool 3,200줄을 통째로 주입 | `/map-db` 를 받아 그리는 캔버스 |
| 지운 것 | | `/map`, `/map-static`, `_extract_map_tool_parts()` |

`_map_tool_dir()` 만 남겼다 — DB 가 없을 때 랜드마크를 파일에서 읽는 폴백이 쓴다.

새 엔드포인트 둘.

| Method | 경로 | 주는 것 |
|---|---|---|
| GET | `/map-db/floors/{id}/graph` | 노드·연결 — **안내에 쓰는 그래프 그대로** |
| GET | `/map-db/floors/{id}/route` | 비콘 순서 (지도에서 확인용) |

### 목적지는 목록이 아니라 **말로** 고른다

화면에 드롭다운을 두면 정작 확인해야 할 구간을 건너뛴다. 말한 문장이 목적지에
제대로 붙는지가 검증 대상인데, 목록에서 고르면 그 단계가 없어진다.

그래서 입력창에 친 문장이 **폰에서 온 음성과 똑같은 메시지**로 나간다.

```jsonc
{ "type":"destination", "event":"resolve", "text":"사백칠호 가줘", "fromBeacon":null }
```

`fromBeacon` 은 폰 없이 경로만 볼 때 출발점을 손으로 지정하는 용도다.
실제 안내에서는 안 온다.

### 검증

```bash
python tests/test_destination_to_route.py    # 말 한마디 → 추적 등록까지 — 9개
```

---

## 9-1. (지난 기록) 재구성 전에 정리해 둔 근거
