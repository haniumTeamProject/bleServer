...
def lead_phrase(lead_m: float) -> str:
    ...
    그래서 배정을 바꾸는 대신 거리를 문장에 담는다. 15m 앞에서 말해도
    "약 15미터 뒤 왼쪽으로 꺾으세요"라면 언제 할 일인지 전달된다.
    ...
    0.7m/s 로 걷는 속도 기준이다.
        ...
        6~15m      9~21초 — "조금 뒤"
        15m 이상   숫자를 준다. 이쯤 되면 어림짐작이 안 되고 걸음 수로 세야 한다
    ...
    if lead_m < 15.0:
        return "조금 뒤 "
    return f"약 {int(round(lead_m / 5.0) * 5)}미터 뒤 "
